          ...
          unfiltered_agg_subset_corrs_shuffles.items()):
            ...
            for (key2x, value2x), (key2s, value2s) in zip(
              value1x.items(), value1s.items()):
                ...
                condition_s.replace(to_replace=np.nan, value=0.0, inplace=True)

                range = len(condition_x)
                for j in np.arange(1000):
                    multiplier = j+1
                    upper = range * multiplier
                    lower = upper-range
                    condition_s[lower:upper] = np.where(
                        np.arange(m)[:, None] < np.arange(n),
                        np.nan, condition_s[lower:upper])
                    np.fill_diagonal(
                        a=condition_s[lower:upper].values,
                        val=np.nan, wrap=True)

                condition_s = condition_s.unstack().sort_values().dropna()

                idx = pd.MultiIndex(levels=[[], []], labels=[[], []])
                uncorrected_pvals = pd.DataFrame(
                    index=idx, columns=['rho', 'two_tailed_pval'])
                for exp_rho, (name, group) in zip(
                  condition_x_final, condition_s.groupby(condition_s.index)):
                    idxl_1 = group.index.get_level_values(level=0)[0]
                    idxl_2 = group.index.get_level_values(level=1)[0]
                    rho = condition_x_final[idxl_1, idxl_2]
                    cutoff = abs(rho)
                    count = len(
                        [i for i in group.values if abs(i) >= cutoff])
                    pval = count/len(group.values)  # (1+count)/(len(group.values)+1)
                    uncorrected_pvals.loc[(idxl_1, idxl_2), :] = [rho, pval]

                uncorrected_pvals['rho'] = uncorrected_pvals['rho'].astype(
                    np.float64)
                ...
                unfiltered_sig_corr_dict[key2x] = sig_corrected_qvals
            print()
...
sig_corrs('unfiltered_subset_corrs')
    ...
    if data_type == 'experimental':
        ...
        for key1, value1 in correlation_matrix_dict_copy.items():
            ...
            for key2, value2 in value1.items():
                ...
                threshold = 0.6*max(linkage_row[:, 2])
                ...
                if idx_full[0].count('_') == 2:
                    ...
                    g = sns.clustermap(
                        ...
                        cmap=cmap, center=0, xticklabels=1,
                        yticklabels=1)
...
def table_dif():
    ...
    for i in master_list:
        ...
        if 'unfiltered' in keys[0]:
            result = result.merge(
                pd.DataFrame(
                    unfiltered_sig_corr_dict[gl261_key]['two_tailed_qval']),
            ...
            result = result.merge(
                pd.DataFrame(
                    unfiltered_sig_corr_dict[naive_key]['two_tailed_qval']),
                how='left', left_index=True, right_index=True)
...
silhouette_analysis(
    'unfiltered_subset_corrs_dropped', 10, 'naive_unfiltered', save_dir)
        ...
        for key2, value2 in value1.items():
            if key2 in ['naive_unfiltered', 'gl261_unfiltered']:
                ...
                g = sns.clustermap(
                    ...
                    cmap=cmap, center=0, xticklabels=1, yticklabels=1)
