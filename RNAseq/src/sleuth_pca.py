...
sleuth_path = '/Users/gjbaker/projects/gbm_immunosuppression/' + \
    'RNAseq/data/sleuth/min2and7'
...
m = pd.read_csv(
    os.path.join(
        sleuth_path, 'normalized_TPM_matrix_min2and7.tsv'), sep='\t')
...
plt.legend(loc='best')
plt.tight_layout()
plt.savefig(os.path.join(sleuth_path, 'kallisto_scree_plot_min2and7' + '.pdf'))
plt.close('all')
...
for p, i in zip([('PC_1', 'PC_2'), ('PC_3', 'PC_4')], [(0, 1), (2, 3)]):
    ...
    plt.savefig(
        os.path.join(
            sleuth_path,
            'salmon_score_plot_min2and7_' + p[0] + 'v' + p[1] + '.pdf'))
    plt.close('all')
