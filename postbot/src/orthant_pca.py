...
class_lut = {'B': '#FF766D', 'CD4T': '#E38900',
             'CD8T': '#7CAE00', 'DC': '#C49A00', 'NK': '#00BFC4',
             'PMN': '#53B400', 'Eo': '#FF66A8', 'Mono': '#00B6EB',
             'Mac': '#06A4FF', 'LTi': '#FB61D7', 'DPT': '#00C094',
             'ISPT': '#00BC56', 'DNT': '#DF70F8',
             'Precursor': '#A58AFF', 'unspecified': '#000000'}

# generate data folder for PCA analysis
pca_savedir = os.path.join(
    project_path, 'postbot', 'data', 'logicle_e20', 'orthant', 'pca_data')
os.makedirs(pca_savedir)

# change directory to read overall.csv (from orthant.py)
...
for column in naive_PC_ranks:
    sample = naive_PC_ranks[column].reindex(
        [naive_PC_ranks[column].values])
      ...
      zip(cell_type_list, major_classes, matrix_w)):
        ...
        if cell_type != 'unspecified':
            ...
            plt.annotate(cell_type_update, xy=(np.asarray(value[x_index]),
                         np.asarray(value[y_index])), xytext=(2, 2), size=1,
